...
func setupUpdate() ([]string, dto.FreeMarketRequestBody, domain.FreeMarket, sqlmock.Sqlmock, *sqlx.DB) {
	cols := []string{
		"id",
		"latitude",
		"longitude",
...
func TestUpdate(t *testing.T) {
	t.Run("should update a free market by id", func(t *testing.T) {
		cols, freeMarketRequest, freeMarketResponse, mock, database := setupCreate()
		defer database.Close()

		mock.ExpectQuery("UPDATE free_market").WithArgs(
		...
		freeMarket, err := sut.Update(ctx, freeMarketResponse.ID, &freeMarketRequest)
	...
	t.Run("should return error on any database error", func(t *testing.T) {
		_, freeMarketRequest, freeMarketResponse, mock, database := setupCreate()
		defer database.Close()
